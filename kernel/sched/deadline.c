...
static inline u64 dl_bw_fair(int i)
{
	return cpu_rq(i)->fair_server.dl_bw;
}
...
void dl_init_tg(struct task_group *tg, int cpu, u64 rt_runtime, u64 rt_period)
{
	 ...
	 * that big.
	 */
	BUG_ON(rt_runtime != 0 && rt_runtime < (1ULL << DL_SCALE));

	/*
	 * Since we use the MSB for wrap-around and sign issues, make
	 * sure it's not set (mind that period can be equal to zero).
	 */
	BUG_ON(rt_period & (1ULL << 63));
...
static enum hrtimer_restart dl_server_timer(struct hrtimer *timer, struct sched_dl_entity *dl_se)
{
	...
	scoped_guard (rq_lock, rq) {
		...
		rq_unpin_lock(rq, rf);		// rt_server_has_tasks might need to pull!!! FIXME: is this safe?
